...
export default function EditCmsPage({
  ...
  // Fetch Existing CMS Page Data
  const { data: pageData, isLoading } = useQuery<CmsPageRecord>({
    queryKey: ["adminCmsPageSingle", id],
    queryFn: async () => {
      // First search in admin list
      const res = await api.get("/admin/cms", { params: { limit: 100 } });
      const found = res.data?.data?.find((p: CmsPageRecord) => p.id === id);
      if (found) return found;
      // Fallback
      const resSingle = await api.get(`/cms/${id}`);
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-28 text-center">
        ...
        <p className="text-xs font-semibold text-muted-foreground">Loading CMS Page Details...</p>
      </div>
    );
  }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-extrabold tracking-tight text-foreground font-heading">
              Edit "{formData.title || "CMS Page"}"
            </h1>
            <p className="text-xs text-muted-foreground mt-0.5">
              Update page details, rich HTML content, and publishing status.
            </p>
          </div>
        </div>
